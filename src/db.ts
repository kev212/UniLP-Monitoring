    ...
    await this.pool.query(`
      ...
      CREATE TABLE IF NOT EXISTS position_groups (
        ...
        shape TEXT NOT NULL CHECK (shape = 'bid_ask'),
         shape_version TEXT NOT NULL CHECK (shape_version IN ('delta-amount-linear-v1', 'delta-amount-linear-v2', 'delta-amount-linear-v3', 'delta-amount-linear-v4')),
       ...
       BEGIN
         IF NOT EXISTS (
           ...
           ALTER TABLE position_groups ADD CONSTRAINT position_groups_shape_version_check
             CHECK (shape_version IN ('delta-amount-linear-v1', 'delta-amount-linear-v2', 'delta-amount-linear-v3', 'delta-amount-linear-v4'));
         END IF;
       END $$;
