...
describe("BSC Uniswap V4 registry", () => {
  it("replaces PancakeSwap with official Uniswap V4 deployments", () => {
    ...
    expect(registry.discoveryProtocols).toEqual(["v4"]);
    ...
    expect(registry.contracts.v3.factory).toBe(zeroAddress);
    expect(isProtocolDeployed(registry, "v3")).toBe(false);
    expect(isProtocolDeployed(registry, "v4")).toBe(true);
    expect(isEligibleScanDex(registry, "uniswap-v4-bsc")).toBe(true);
    expect(isEligibleScanDex(registry, "uniswap-v3-bsc")).toBe(false);
